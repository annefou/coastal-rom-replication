...
| FORRT Outcome's `Repository URL` field | Points to `https://github.com/annefou/coastal-rom-replication`. |
...
| GHCR Docker pull | `docker pull ghcr.io/annefou/coastal-rom-replication:latest` over HTTPS. |
...
- [ ] Docker image builds and runs the snakemake target via `docker run --rm ghcr.io/annefou/coastal-rom-replication:latest`.
